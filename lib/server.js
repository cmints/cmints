...
function onRequest(req, res)
{
  ...
  const hasGzip = headers["accept-encoding"] && /\bgzip\b/.test(acceptEncoding);
          ...
          (isTranslated(page, locale) || locale === defaultLocale))
  {
    parsePage(page, ext, locale).then((html) =>
    {
      ...
      if (isCache)
      {
        ...
        cacheFile(path.join(contentDir, locale, page) + ".html", readableData);
      }
    }).catch((reason) =>
    {
...
let generateStatic = (callback) =>
{
  ...
  Promise.all([pages, locales, public]).then(([files, locales, publics]) =>
  {
    ...
    const stackItems = [...files, ...locales, ...publics];
    // Double add for gzip
    staticStack = [...stackItems, ...stackItems];
...
function getCachedFilePath(requestPath, ext, locale, hasGzip)
{
  ...
  const htmlPath = path.join(contentDir, locale, requestPath) + ".html";
