...
const {pathnameToPath} = require("./custom-utils");
...
const {pageExtensions, multiLang, gzip, i18nOptions} = config;
const {defaultLocale, detectLang} = i18nOptions;
...
const resourcesMap = {
  ...
  ".xml": {encoding: "utf-8", type: "application/xml"},
  ...
  ".jpeg": {encoding: "binary", type: "image/jpeg"},
  ...
  ".mp3": {encoding: "binary", type: "audio/mpeg"},
  ".ogg": {encoding: "binary", type: "audio/ogg"}
};
...
{
  const requestLocale = getLocaleFromPath(url.parse(req.url).pathname);
  if (!multiLang)
  {
  ...
  {
    if (requestLocale === defaultLocale && i18nOptions.type === "Index")
      req.show404 = true;

    return [requestLocale, requestLocale];
  }
  // Use accept-language header
  else if (detectLang && !isStatic)
  {
    const acceptLanguage = getHeaders(req)["accept-language"];
    const pageLocales = getPageLocales(page);
    const detectedLocale = getLocaleFromHeader([defaultLocale, ...pageLocales],
                                               acceptLanguage);
    return [detectedLocale, detectedLocale];
  }
  else
  {
    return [defaultLocale, ""];
...
function resolveResourceData(pathnameWithoutLocale)
{
  ...
  if (getPermalinkedPage(pathnameWithoutLocale))
  {
    permalink = pathnameWithoutLocale;
    page = getPermalinkedPage(pathnameWithoutLocale);
  }
...
{
  const pathnameNoLocale = getPathnameWithoutLocale(req);
  const isPermalinked = isPagePermalinked(pathnameToPath(pathnameNoLocale));
  const parsedPath = path.parse(pathnameNoLocale);
  const {dir, name, base} = parsedPath;
  const isPageExtensionRequested = pageExtensions.includes(parsedPath.ext);
  const isPublicFile = fileExistsSync(path.join(publicDir, dir, base));
  const {page, ext, encoding, permalink,
    type} = resolveResourceData(pathnameNoLocale);
  const isDraft = name.startsWith("_") &&
                  fileExistsSync(path.join(pageDir, page + ext));
  const [locale, cacheLocale] = getLocale(req, page);
  const cachePath = getCachedFilePath(page, ext, locale, acceptGzip(req));
  const urlLocale = getLocaleFromPath(url.parse(req.url).pathname);
  const isLocalesDirExist = fs.existsSync(localesDir);

  if (name == 404)
    res.statusCode = 404;

  // Do not allow index in the address and permalinked pages
  if (base == "index" || isPermalinked || (isDraft && !showDraft) ||
      req.show404)
  {
    delete req.show404;
  ...
  else if (pageExtensions.includes(ext) &&
          (!multiLang || isTranslated(page, locale) ||
          locale == defaultLocale))
  {
    parsePage(page, ext, locale, urlLocale).then((html) =>
    {
      if (isLocalesDirExist)
      ...
      if (isCache)
      {
        ...
        const filePath = path.join(contentDir, cacheLocale, location) +
                         staticExt;
...
let generateStatic = (callback) =>
{
  ...
  Promise.all([pages, locales, publics]).then(([files, locales, publics]) =>
  {
    ...
    {
      if (i18nOptions.type === "Double")
      {
        locales.push("");
      }
      else
      {
        locales[defaultLocaleIndex] = "";
      }
    }
...
{
  return pageExtensions.filter((ext) =>
