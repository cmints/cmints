...
const {pageExtestions, defaultLocale, multiLang} = config;
...
let runServer = (argv) =>
{
  ...
  if (isHttps)
  {
    ...
    server = require("http2").createSecureServer(options, onRequest);
  }
  else
  {
...
function onRequest(req, res)
{
  ...
  const hasGzip = acceptEncoding && /\bgzip\b/.test(acceptEncoding);
  ...
  if (!encoding || !type)
  {
    writeError(res, 501);
          ...
          (cachePath = getCachedFilePath(page, ext, locale, hasGzip)))
  {
    ...
    fileStat(cachePath).then((stats) => 
    {
      ...
      writeResponse(res, readStream, encoding, type);
          ...
          (!multiLang || isTranslated(page, locale) || locale === defaultLocale))
  {
    parsePage(page, ext, locale).then((html) =>
    {
      ...
      if (isCache)
      {
        ...
        cacheFile(path.join(contentDir, locale, page) + staticExt, readableData);
      }
    }).catch((reason) =>
    {
...
let generateStatic = (callback) =>
{
  ...
  Promise.all([pages, locales, public]).then(([files, locales, publics]) =>
  {
    ...
    for (const public of publics)
    {
      const publicUrl = public.replace(publicDir, "");
