...
const {pathnameFromfilePath, getPath} = require("./custom-utils");

const {pageDir} = require("../config").dirs;

const pageDatas = [];
const permalinktoPageMap = {};
  ...
  glob(`${pageDir}/**/*.*`, {}).then((filePaths) =>
  {
    ...
    for (const filePath of filePaths)
    {
      readFile(filePath, "utf8").then(frontMatter).then((data) =>
      {
        ...
        {
          // Add to permlinks
          const pathname = pathnameFromfilePath(filePath);
          permalinktoPageMap[data.attributes.permalink] = pathname;
        }

        data.attributes.path = getPath(filePath, data.attributes);
...
module.exports = {initSitedata, queryPages, getPermalinkedPage, isPagePermalinked};
