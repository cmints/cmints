...
const initBrowserify = function(inputDirPath, outputDirPath, callback)
{
  ...
  {
    prepareBrowserify(inputDirPath, outputDirPath, callback);
    for (let folder of folders)
    {
      // {recursive: true} is only supported in MacOS and Windows
      fs.watch(folder, {}, () =>
      {
        prepareBrowserify(inputDirPath, outputDirPath);
      });
    }
  });
};
 ...
 *                                  * Boolean, true if Processed successfully
 */
function prepareBrowserify(inputDirPath, outputDirPath, callback)
{
