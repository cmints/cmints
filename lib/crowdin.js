...
function removeFilesFromCrowdin(files)
{
  ...
  {
    request.post({
      url: `${crowdinApi}/${projectId}/delete-file?${query}`,
      form: {file}
    }, (err, resp, body) =>
    {
      const response = JSON.parse(body);
...
function addUpdateSourceFiles()
{
  ...
  request(`${crowdinApi}/${projectId}/info?${query}`, (error, response, body) =>
  {
      ...
      const getAllCrowdinStrings = (files, dir = "/") =>
      {
        for (const file of files)
        {
          if (file.node_type == "file")
            crowdinFilesToRemove.push(dir + file.name);
          else if (file.node_type == "directory")
            getAllCrowdinStrings(file.files, dir + file.name + "/");
        }
      };
      getAllCrowdinStrings(JSON.parse(body).files);
