...
/**
 * Get's the success tag status value from the crowdin api call response
 * @param {String} body the response from the crowdin server
 */
function getStatus(body)
{
  return /<success status="(\w+)"\/>/.exec(body);
}
...
const runCrowdinSync = (argv) =>
{
  ...
  switch (argv.crowdin)
  {
    ...
    case "get-translations":
      request(`${crowdinApi}/${projectId}/export?key=${projectKey}`, (error, response, body) =>
      {
        if (error)
        {
        ...
        {
          console.log(`Project build has been ${getStatus(body)[1]}`);
