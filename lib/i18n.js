 ...
 * @param  {String} sourceMessage (optional) original message
...
function parseMessage(text, locale, sourceMessage)
{
  ...
  while ((match = tagSelector.exec(text)) != null)
  {
    ...
    let sourceAttributesObj;
    if (sourceTagsMap)
      sourceAttributes = getDeepValue([tagName, tagNumber - 1], sourceTagsMap);
    ...
    if (sourceAttributes)
    {
      ...
      for (let [attr, value] of sourceAttributes)
      {
        ...
        attributes += value ? `${attr}="${escapeQuotes(value)}"` : attr;
      }
    }
