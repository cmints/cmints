...
const allowedTags = ["a", "img", "p", "span", "div", "em", "i", "b", "strong", "code"];
 ...
 */
let init = function(localesDirPath, watchDirs, callBack)
{
...
let translate = function(html, pagePath, locale)
{
  ...
  while ((match = translationSelector.exec(html)) != null)
  {
    ...
    {
      localePath = localePath.substring(1, --localePath.length); // remove ()
...
/**
 * Generates JSON object using with the translation strings
...
const generateSourceJson = function(html)
{
  ...
  while ((match = translationSelector.exec(html)) != null)
  {
    ...
    description += updateDescriptionTags(message, "fix", "Fixed placeholders:");
    description += updateDescriptionTags(message, "a", "anchor placeholders:");
    message = message.trim();
    result[stringId] = {message: message, description: description};
  }
  return result;
};

function updateDescriptionTags(message, tagName, heading)
{
  let description = "";
  let count = 1;
  ...
  while ((match = regExp.exec(message)) != null)
  {
    ...
    description += `<fix${count}> - ${fixValue} \n`
    count ++;
  }
  if (description)
    description = `\n\n ${heading} \n ${description}`;
...
{
  let attributes = [];
  let linkLocale = isTranslated(pagePath, locale) ? locale : defaultLocale;
  pagePath = `/${linkLocale}/${pagePath}`;
...
function parseFixTags(sourceMessage, localMessage)
{
  ...
  while ((match = fixSelector.exec(sourceMessage)) != null)
  {
    let [fixMatch, fixValue] = match;
    fixes.push(fixValue);
    sourceMessage = cicleReplace(fixSelector, sourceMessage, fixMatch, fixValue);
  }
  if (!localMessage)
...
{
  const result = translationSelector.exec(text);
  // Reset capture position
  translationSelector.lastIndex = 0;
...
exports.getPageLocales = getPageLocales;
exports.hrefAndLang = hrefAndLang;
exports.getStringId = getStringId;
exports.defaultLocale = defaultLocale;
exports.generateSourceJson = generateSourceJson;
exports.isTranslated = isTranslated;
