  ...
  while ((match = translationSelector.exec(html)) != null)
  {
    ...
    if (message)
    {
      ...
      if (locale != defaultLocale)
      {
        ...
        if (localMessage)
          message = parseMessage(localMessage, locale, createTagsMap(message));
      }
    }
    else if (filePath) // Reference to separate translation file
    {
      ...
      if (!message)
        ...
        message = parseMessage(message, locale);
    }
    else
    {
 ...
 */
function parseMessage(text, locale, tagsMap)
{
  ...
  while ((match = tagSelector.exec(text)) != null)
  {
    ...
    {
      let sourceAttributes = Object.entries(tagsMap[tagName][tagNumber - 1])
      for (let [attr, value] of sourceAttributes)
      {
        // Page attribute is used to generate link to the local page and use
        // Default locale as a fallback
        if (attr == "page" && tagName == "a")
        {
          attr = "href";
          ...
          sourceAttributes.push(["hreflang", linkLocale]);
        }
        attributes += " "; // keep space between arguments
        attributes += value ? `${attr}=${value}` : attr;
      }
    }
...
function createTagsMap(text)
{
  ...
  while ((match = tagSelector.exec(text)) != null)
  {
    ...
    let attributesMap = attributes.trim().split(" ").reduce((acc, item) =>
    {
      if (!item)
        return acc;

      let [attributeName, value] = item.split("=");
      acc[attributeName] = value ? value.replace(/['"]+/g, "") : "";
      return acc;
    }, {});
